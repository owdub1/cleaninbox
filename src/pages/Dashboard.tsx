...
const Dashboard = () => {
  ...
  const isOnetime = subscription.plan.toLowerCase() === 'onetime';
          ...
          <div className="mt-6">
                  ...
                  <div className="p-6">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between pb-4 border-b border-gray-200 dark:border-gray-700">
                      ...
                      {userData.subscription.nextBilling && <div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">{isCancelled ? 'Access Until' : isOnetime ? 'Expires' : 'Next Billing'}</p>
                        <div className="flex items-center">
            ...
            {activeTab === 'settings' && <div>
                ...
                <div className="mt-6 bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden">
                  ...
                  <div className="p-6">
                    <div className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-md p-4 mb-6">
                      <div className="flex">
                        ...
                        <div className="ml-3">
                          ...
                          <div className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                            <p>Download all your data as a JSON file (email accounts, senders, activity log, and subscription info).</p>
                          </div>
                          <div className="mt-4">
                            <button
                              type="button"
                              onClick={async () => {
                                try {
                                  const res = await fetchWithAuth('/api/user/export-data', { method: 'GET' }, refreshToken);
                                  if (!res.ok) throw new Error('Export failed');
                                  const blob = await res.blob();
                                  const url = URL.createObjectURL(blob);
                                  const a = document.createElement('a');
                                  a.href = url;
                                  a.download = `cleaninbox-data-${new Date().toISOString().split('T')[0]}.json`;
                                  a.click();
                              ...
                              className="inline-flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-gray-600 font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 text-sm"
                            >
