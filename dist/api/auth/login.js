...
import { comparePassword, getClientIP, getUserAgent } from '../lib/auth-utils.js';
 ...
 */
async function generateRefreshToken(userId, ipAddress, userAgent, rememberMe = false) {
    // Determine expiry based on Remember Me option
    const expiryDays = rememberMe ? 30 : 7;
    const expiryTime = rememberMe ? '30d' : '7d';
    // Generate refresh token
    const refreshToken = jwt.sign({ userId }, JWT_REFRESH_SECRET, { expiresIn: expiryTime });
 ...
 */
async function checkAndUnlockAccount(userId) {
 ...
 */
async function recordLoginAttempt(userId, email, ipAddress, userAgent, successful, failureReason) {
    await supabase
        .from('login_attempts')
        .insert([{
            user_id: userId,
 ...
 */
async function handleFailedLogin(userId, email, ipAddress, userAgent, reason) {
