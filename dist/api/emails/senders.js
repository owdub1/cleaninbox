...
export default async function handler(req, res) {
    ...
    if (!user)
        return;
    // Query parameters
    const { email, // Filter by specific email account
    sortBy = 'count', // 'count', 'name', 'date'
    sortDirection = 'desc', limit = '100', offset = '0', filter // 'newsletter', 'promotional', 'unsubscribable'
     } = req.query;
    try {
        // Build query
        let query = supabase
      ...
      `)
        ...
        // Transform response
        const response = (senders || []).map((sender) => ({
            id: sender.id,
