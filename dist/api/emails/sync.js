...
import { fetchSenderStats } from '../lib/gmail-api.js';
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const { accessToken } = await getValidAccessToken(user.userId, account.gmail_email || email);
        ...
        const sendersToUpsert = senderStats.map((sender) => ({
            user_id: user.userId,
        ...
        const totalEmails = senderStats.reduce((sum, s) => sum + s.count, 0);
        await supabase
            .from('email_accounts')
            .update({
